...
from .deformable_transformer_debug import build_deforamble_transformer
import copy
...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # print(len(features[0].tensor))
        # print(features[0].tensors[0])
        # show_tensor(features[0].tensors[0])
...
#     tensor_cpu = tensor_cpu[:64, :, :]
#     tensor_cpu = (tensor_cpu - tensor_cpu.min()) / \
#         (tensor_cpu.max() - tensor_cpu.min())
#     tensor_cpu = (tensor_cpu * 255).astype(np.uint8)
#     # transpose the tensor to match the expected shape for 3D plotting
#     tensor_cpu = np.transpose(tensor_cpu, (2, 1, 0))
#     # print(tensor_cpu[:3, :, :])
...
#     # loop through the channels and plot each one on a subplot
#     for i in range(64):
#         ax[i // 8, i % 8].imshow(tensor_cpu[:, :, i], cmap='gray')
#         ax[i // 8, i % 8].axis('off')

#     # save the figure as 3 separate images
