...
class BackboneEfficientNetV2(nn.Module):
    def __init__(self, train_backbone):
        ...
        inverted_residual_setting, last_channel = _efficientnet_conf(
            "efficientnet_v2_s")
        efficentnet_model = _efficientnet(inverted_residual_setting, 0.2, last_channel,
                                          weights, progress=True, norm_layer=FrozenBatchNorm2d,)

        for name, parameter in efficentnet_model.named_parameters():
...
def build_efficientnet_backbone(args):
    position_embedding = build_position_encoding(args)
    train_backbone = args.lr_backbone > 0
...
def build_swin_fixed_backbone(args):
    position_embedding = build_position_encoding(args)
    train_backbone = args.lr_backbone > 0
    return_interm_layers = args.masks or (args.num_feature_levels > 1)
    backbone = BackboneSwinTransformerFixed()(train_backbone, return_interm_layers)
    model = Joiner(backbone, position_embedding)
    return model